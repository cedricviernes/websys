...
<div class="container" style="max-width: 600px; margin: 2rem auto; background: #fff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); padding: 2rem;">
    <h1 style="margin-bottom: 1.5rem;">Edit Discount</h1>
    ...
    <form action="{{ route('admin.discounts.update', $discount) }}" method="POST">
        ...
        <div class="mb-3" style="margin-bottom: 1rem;">
            ...
            <input type="text" name="code" id="code" class="form-control" style="width: 100%; padding: 0.5rem; border-radius: 4px; border: 1px solid #e5e7eb;" value="{{ old('code', $discount->code) }}" required>
        </div>

        <div class="mb-3" style="margin-bottom: 1rem;">
            <label for="amount" class="form-label" style="font-weight: 500;">Amount</label>
            <input type="number" name="amount" id="amount" class="form-control" style="width: 100%; padding: 0.5rem; border-radius: 4px; border: 1px solid #e5e7eb;" value="{{ old('amount', $discount->amount) }}" step="0.01" required>
        </div>
        ...
        <div class="mb-3" style="margin-bottom: 1rem;">
            ...
            <input type="date" name="valid_from" id="valid_from" class="form-control" style="width: 100%; padding: 0.5rem; border-radius: 4px; border: 1px solid #e5e7eb;" value="{{ old('valid_from', $discount->valid_from) }}">
        </div>

        <div class="mb-3" style="margin-bottom: 1.5rem;">
            <label for="valid_until" class="form-label" style="font-weight: 500;">Valid Until</label>
            <input type="date" name="valid_until" id="valid_until" class="form-control" style="width: 100%; padding: 0.5rem; border-radius: 4px; border: 1px solid #e5e7eb;" value="{{ old('valid_until', $discount->valid_until) }}">
        </div>

        <div style="display: flex; gap: 1rem;">
            <button type="submit" class="btn btn-primary" style="background: #2563eb; color: #fff; border: none; padding: 0.5rem 1.5rem; border-radius: 4px;">Update Discount</button>
